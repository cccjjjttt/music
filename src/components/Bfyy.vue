...
export default {
    ...
    created() {
        ...
        console.log('cc',this.c)

                this.axios({
        method: 'POST',
            url: 'https://api.itooi.cn/music/kuwo/songList?key=579621905&id='+this.$route.params.a,
      }).then(result => {
          this.na = result.data.data.songs[this.index].name;
           this.nt = result.data.data.songs[this.index].singer;
           this.c = result.data.data.songs[this.index].pic;
           this.u=result.data.data.songs[this.index].url;
            this.t=result.data.data.songs[this.index].time;
            this.ty=result.data.data.songs.length;
    ...
    methods: {
        ...
        go(){
            if(this.active==0){
                     this.$router.push({name:'Wogd',params:{a:this.a,active:this.active}})
            }else if(this.active==1){
                  this.$router.push({name:'Pg',params:{active:this.active,b:this.b,a1:this.a1}})
